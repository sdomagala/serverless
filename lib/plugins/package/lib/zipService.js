...
module.exports = {
  ...
  zipFiles(files, zipFileName, prefix) {
    ...
    // remove any slash if present
    const sanitizedPrefix = prefix &&  prefix.replace(/(.+)[\\\/]$/, '$1');
    ...
    return new BbPromise((resolve, reject) => {
      ...
      output.on('open', () => {
        ...
        const filePaths = files.map(file => (sanitizedPrefix ? path.join(sanitizedPrefix, file) : file));
        BbPromise.all(filePaths.map(this.getFileContentAndStat.bind(this))).then((contents) => {
          _.forEach(_.sortBy(contents, ['filePath']), (file) => {
            const name = file.filePath.slice(sanitizedPrefix ? `${sanitizedPrefix}${path.sep}`.length : 0);
